...
function activate(context) {
	...
	let disposable_settings = vscode.commands.registerCommand('ai-dvm.opensettings', function () {
		vscode.commands.executeCommand('workbench.action.openSettings', 'AI-dvm');
	});

	let disposable_setprompt = vscode.commands.registerCommand('ai-dvm.setprompt', async function () {
		...
		if (prompt) {
            message = prompt;
            vscode.window.showInformationMessage(`AI-dvm включен. Задача: ${prompt}`);
        } else {
	...
	let disposable_generate = vscode.commands.registerCommand('ai-dvm.generate_responce', async function () {
		const ExtensionConfig = vscode.workspace.getConfiguration('AI-dvm configuration');
	...
	let disposable_print = vscode.commands.registerCommand('ai-dvm.print_responce', async function () {
		if (global_response == " "){
